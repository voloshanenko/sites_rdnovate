...
// back to Shifting Pixel (either this page or shiftingpixel.com), but donӴ worry about
// including a big link on each page if you donӴ want toׯne will do just nicely. Feel
...
define('MEMORY_TO_ALLOCATE',	'10M');
define('DEFAULT_QUALITY',		90);
define('CURRENT_DIR',			dirname(__FILE__));
define('CACHE_DIR_NAME',		'/cache/mod_thumbsup/');
define('DOCUMENT_ROOT',			$_SERVER['DOCUMENT_ROOT']);
// Strip the possible trailing slash off the document root

$docRoot=preg_replace('/\/$/', '',DOCUMENT_ROOT);

$cacheRoot	= preg_replace('/\/$/', '', str_replace("/modules/mod_thumbsup/image.php","",$_SERVER["SCRIPT_FILENAME"]));
define('CACHE_DIR',$cacheRoot.CACHE_DIR_NAME);
...
$params=split("@",$_GET["parameters"]);
foreach($params as $param){
//print $param;
	list($var,$val)=split("=",$param);
	switch($var){
		case "image":
		$image=preg_replace('/^(s?f|ht)tps?:\/\/[^\/]+/i', '', (string) $val);
		break;
		case "height":
		$getheight=$val;
		break;
...
if (isset($getcolor))
	$color		= preg_replace('/[^0-9a-fA-F]/', '', (string) $getcolor);
else
	$color		= FALSE;
...
$quality	= (isset($getquality)) ? (int) $getquality : DEFAULT_QUALITY;
...
if (isset($getcropratio))
	$resizedImageSource	.= 'x' . (string) $getcropratio;
...
if (!isset($getnocache) && file_exists($resized))
{
...
switch ($size['mime'])
{
	...
	default:
		...
		$mime				= 'image/jpeg'; // We need to convert GIFs to PNGs
		$doSharpen			= FALSE;
		$quality			= $quality; // We are converting the GIF to a PNG and PNG needs a compression level of 0 (no compression) through 9
//$doSharpen			= TRUE;
	break;
}
