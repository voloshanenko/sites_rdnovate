		...
		if ($this->createGroupsTable()) {
			...
			if (!$db->loadResult()) {
				...
				$tmpl = array(
					...
					'fullscreen,preview,print,visualaid,style,xhtmlxtras,anchor,unlink,advlink,imgmanager,spellchecker,advcode,article,contextmenu,inlinepopups'
				);
	...
	function installPlugins($install = false)
	{
		...
		if ($this->createPluginsTable()) {
			...
			if (!$db->loadResult()) {
				...
				$plugins = array('contextmenu','directionality','emotions','fullscreen','paste','preview','table','print','searchreplace','style','layer','nonbreaking','visualchars','xhtmlxtras','imgmanager','advlink','spellchecker','help','browser','inlinepopups','media','advcode','article','code','media');
