...
* See readme.html.
* @license		GNU/GPL
ANY JS code at this file is not licensed under the GPL license and you should get a written permission from webmaster@chronoengine.com to use it!
...
class HTML_ChronoContact {
		...
		<tr>
            ...
            <tbody>
				...
				foreach($results as $result){
					...
					<?php echo ${$result}->result_TITLE; ?></a>
                    </td></tr>
					<?php
  ...
  function editChronoContact( $row, $option ) {
	...
	<table class="admintable">
  	<tr>
		...
		<td></td>
		<td><input type="text" class="inputbox" size="50" maxlength="100" name="name" id="cf_adminform_name" value="<?php echo $row->name; ?>" /></td>
	</tr>
    <tr>
    ...
    <fieldset class="adminform">
	<legend><?php echo JText::_( 'Core/View Form Settings' ); ?></legend>
	<table class="admintable">	
	<tr>
    ...
    <fieldset class="adminform">
        <legend><?php echo JText::_( 'Form URLs' ); ?></legend>
        <table class="admintable">	
        <tr>
            ...
            <td></td>
            <td><input type="text" class="inputbox" size="50" name="redirecturl" value="<?php echo htmlspecialchars($row->redirecturl); ?>" />
            <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::by default(if empty) will load an empty body area">
			<?php echo HTML_ChronoContact::WarningIcon(); ?>
        	</span>
            </td>
        </tr>    
        <tr>
            ...
            <td></td>
            <td><input type="text" class="inputbox" size="50" name="submiturl" value="<?php echo htmlspecialchars($row->submiturl); ?>" />
            <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::don't put anything here unless you know what you are doing">
			<?php echo HTML_ChronoContact::WarningIcon(); ?>
        	</span>
            </td>
        </tr>
	...
	<fieldset class="adminform">
	<legend><?php echo JText::_( 'Other Form Settings' ); ?></legend>
	<table class="admintable">
	<tr>
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "Error message which will appear in case of exhausted submissions limit" ); ?></td>
		<td class="key">Submissions Limit error</td>
		<td></td>
		<td>
		<input type="text" name="params[submissions_limit_error]" id="params[submissions_limit_error]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('submissions_limit_error', 'Sorry but you can not submit the form again very soon like this!'); ?>">
		</td>
	</tr> 
	<tr>
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "Enable the Joomla checkToken function once the form is submitted." ); ?></td>
		<td class="key"><strong>checkToken:</strong> </td>
		<td></td>
		<td>
			<select name="params[checkToken]" id="params[checkToken]">				
				<option<?php if($paramsvalues->get('checkToken') == '1'){ echo ' selected';} ?> value="1">ON</option>
                <option<?php if($paramsvalues->get('checkToken') == '0'){ echo ' selected';} ?> value="0">OFF</option>
			</select>
		</td>
	</tr>
	<tr>
    ...
    <tr>
	   <td><?php echo JHTML::_('tooltip', "Chronoforms will try to republish all your fileds with submitted data in case of wrong captcha code submission by user or any error occured and form reshown while posted data array not empty"); ?></td>
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "Enter the list of fields to get skipped when the form is trying to republish the data" ); ?></td>
		<td class="key">Republish skip list</td>
		<td></td>
		<td>
		<input type="text" name="params[captcha_dataload_skip]" id="params[captcha_dataload_skip]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('captcha_dataload_skip'); ?>">
		</td>
	</tr> 
    <tr>
	   <td><?php echo JHTML::_('tooltip', "Choose wheather Chronoforms will renew the form instance when its submitted ? using the same instance may be useful to edit some form data/settings in the extra code boxes before going into the submit routine!"); ?></td>
		<td class="key">Renew Form Instance On Submit</td>
        <td></td>
	   <td>
	   <select name="params[useCurrent]" id="params[useCurrent]">
            <option<?php if($paramsvalues->get('useCurrent') == ''){ echo ' selected';} ?> value="">New Instance</option>
            <option<?php if($paramsvalues->get('useCurrent') == '1'){ echo ' selected';} ?> value="1">Same Instance</option>
        </select>
	   </td>
	</tr>
    <!--
    <tr>
		<td><?php echo JHTML::_('tooltip', "choose if Chronoforms should parse the HTML when the form is saved to generate fields names or email templates, this may make errors if your form code has strange PHP or JS code!" ); ?></td>
		<td class="key"><strong>Parse HTML on save:</strong> </td>
		<td></td>
		<td>
			<select name="params[parsehtml]" id="params[parsehtml]">
				<option<?php if($paramsvalues->get('parsehtml') == '1'){ echo ' selected';} ?> value="1">ON</option>
				<option<?php if($paramsvalues->get('parsehtml') == '0'){ echo ' selected';} ?> value="0">OFF</option>
			</select>
		</td>
	</tr>
    -->
    ...
    <fieldset class="adminform">
	<legend><?php echo JText::_( 'Date Fields Settings' ); ?></legend>
	<table class="admintable">   
    <tr>
		...
		<td></td>
		<td>
		<input type="text" name="params[datefieldformat]" id="params[datefieldformat]" class="inputbox" size="50" maxlength="100" value="<?php echo $paramsvalues->get('datefieldformat', 'd/m/Y'); ?>">
		</td>
	</tr>	
    <tr>
    ...
    <tr>
		...
		<td></td>
		<td>
		<input type="text" name="params[datefieldextras]" id="params[datefieldextras]" class="inputbox" size="100" value="<?php echo $paramsvalues->get('datefieldextras', "classes: ['dashboard']"); ?>">
		</td>
	</tr>
    </table>
    </fieldset>
  ...
  <td valign="top">		
	...
	<div id="page-codes">
				...
				<div id="left_column33">
					...
					?>
						...
						<textarea class="<?php if($emailparams->get('editor') == '1'){ ?>2mce_editable<?php } ?>" id="editor_email_<?php echo $emailscounter; ?>" name="editor_email_<?php echo $emailscounter; ?>" rows="20" cols="75" style="width:90%; height:350px; "><?php echo $email->template; ?></textarea>
	...
	<div id="page-codes">
	<fieldset class="adminform">
	<legend><?php echo JText::_( 'Main onLoad/View Code' ); ?></legend>
	<table class="admintable">
    <tr>
		<td></td>
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "you can use your CSS code here to style your own form or override Chronoforms' css code, please don't use any style tags" ); ?></td>
		...
		<td><textarea name="stylecode" id="stylecode" style="display:none" cols="80" rows="10"><?php echo htmlspecialchars($row->stylecode); ?></textarea></td>
	</tr>
	</table>
    </fieldset>
    <fieldset class="adminform">
	<legend><?php echo JText::_( 'onSubmit Events Code' ); ?></legend>
	<table class="admintable">
    <tr>
		<td></td>
		<td class="key" style="width:250px">Hint:</td>
    ...
    <fieldset class="adminform">
	<legend><?php echo JText::_( 'Extra Code' ); ?></legend>
	<table class="admintable">
    <tr>
		<td></td>
	...
	<div id="page-titlesx">
	<fieldset class="adminform">
	<legend><?php echo JText::_( 'Data Saving/Logging Settings' ); ?></legend>
	<table class="admintable">
	<tr>
    ...
    <fieldset class="adminform">
	<legend><?php echo JText::_( 'DataView Fields' ); ?></legend>
	<table class="admintable">
	<tr>
	...
	<tr style="background-color:#fff ">
		...
		<td class="key"><strong><?php echo 'main view record'; ?>:</strong> </td>
		<td></td>
		<td>
	...
	<div id="page-autogen">
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'AutoGenerated Code/Data' ); ?></legend>
    <table class="admintable">	
    <tr>
		<td></td>
		<td class="key" style="width:250px">Hint</td>
	...
	<tr>
		...
		<td><textarea name="autogenerated" style="display:none" id="autogenerated" cols="80" rows="20"><?php echo htmlspecialchars($row->autogenerated); ?></textarea></td>
	</tr>
	<tr>
	...
	<div id="page-uploads">
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'Files Upload Settings' ); ?></legend>
    <table class="admintable">	
	<tr>
	...
	<tr>
		<td><?php echo JHTML::_('tooltip', "Which fields are of type FILE ? Use this syntax to control allowable extensions : file_2:jpg|png{222-1},file_3:gz|zip{99-0}" ); ?></td>
        ...
        <td>
        <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::E.g: file_2:jpg|png{222-1},file_3:gz|zip{99-0} and all extensions should be in low case!">
			<?php echo HTML_ChronoContact::WarningIcon(); ?>
        </span>
        </td>
	</tr>   
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "PHP code to format the file name, use this carefully as it may break the form" ); ?></td>
		<td class="key" style="width:250px">FileName format</td>
		<td></td>
		<td>
		<input type="text" name="params[filename_format]" id="params[filename_format]" class="inputbox" size="100" value="<?php echo $paramsvalues->get('filename_format', "\$filename = date('YmdHis').'_'.\$chronofile['name'];"); ?>">
		</td>
        <td>
        <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::This field should contain valid PHP code formula without tags or it may break the whole form!">
			<?php echo HTML_ChronoContact::WarningIcon(); ?>
        </span>
        </td>
	</tr> 
    <tr>
    <td></td>
    <td colspan="3" class="key" style="text-align:left">Default Path:<strong><?php echo JPATH_SITE.DS."components".DS."com_chronocontact".DS.'uploads'.DS.$row->name.DS; ?></strong></td>
    <td></td>
    </tr>
	</table>
    </fieldset>
    <fieldset class="adminform">
    <legend><?php echo JText::_( 'Files Upload Erros' ); ?></legend>
    <table class="admintable">	
    <tr>
		<td><?php echo JHTML::_('tooltip', "Error message which will appear in case of exceeded size limit" ); ?></td>
		<td class="key">Size Exceeded error</td>
		<td></td>
		<td>
		<input type="text" name="params[upload_exceedslimit]" id="params[upload_exceedslimit]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('upload_exceedslimit', 'Sorry, Your uploaded file size exceeds the allowed limit.'); ?>">
		</td>
	</tr>
    <tr>
		<td><?php echo JHTML::_('tooltip', "Error message which will appear in case of small files sizes" ); ?></td>
		<td class="key">Size less error</td>
		<td></td>
		<td>
		<input type="text" name="params[upload_lesslimit]" id="params[upload_lesslimit]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('upload_lesslimit', 'Sorry, Your uploaded file size is less than the allowed limit'); ?>">
		</td>
	</tr>
    <tr>
		<td><?php echo JHTML::_('tooltip', "Error message which will appear in case of wrong files types uploaded" ); ?></td>
		<td class="key">Type not allowed error</td>
		<td></td>
		<td>
		<input type="text" name="params[upload_notallowed]" id="params[upload_notallowed]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('upload_notallowed', 'Sorry, Your uploaded file type is not allowed'); ?>">
		</td>
	</tr>
    </table>
    </fieldset>
	...
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'Captcha Settings' ); ?></legend>
    <table class="admintable">
	<tr>
	...
	<tr>
		...
		<td></td>
		<td>
		<input type="text" name="params[imgver_error_msg]" id="params[imgver_error_msg]" class="inputbox" size="50" value="<?php echo $paramsvalues->get('imgver_error_msg', 'You have entered an incorrect verification code at the bottom of the form.'); ?>">
		</td>
	</tr>
    </table>
    </fieldset>
    <fieldset class="adminform">
    <legend><?php echo JText::_( 'GD Info' ); ?></legend>
    <table class="admintable">
	<tr>
	   <td></td>
   ...
   <td><img src='<?php echo $mainframe->getSiteURL(); ?>components/com_chronocontact/chrono_verification.php<?php if($paramsvalues->get('imtype') == '1'){ echo '?imtype=1';}else{ echo '?imtype=0';} ?>'></td>
	...
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'Javascript Live Validation' ); ?>
    <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::<?php echo JText::_( 'This is a Javascript validation which will work while the user is filling the form and before the form is submitted, it may not work if your form has any JS errors or if the user has JS disabled at his browser and its not the best way of validation if the data collected are critical!' ); ?>">
		<?php echo HTML_ChronoContact::WarningIcon(); ?>
    </span>
    </legend>
    <table class="admintable">
	...
	<tr>
		...
		<td class="key"><?php echo "Validation Library"; ?></td>
		<td></td>
		<td>
    ...
    <tr>
		<td><?php echo JHTML::_('tooltip', "whether you want it to validate as you type or only on blur" ); ?></td>
		<td class="key">Run Validation onlyOnBlur</td>
		<td></td>
		<td>
		<select name="params[validate_onlyOnBlur]" id="params[validate_onlyOnBlur]">
			<option<?php if($paramsvalues->get('validate_onlyOnBlur', '1') == '0'){ echo ' selected';} ?> value="0">No</option>
			<option<?php if($paramsvalues->get('validate_onlyOnBlur', '1') == '1'){ echo ' selected';} ?> value="1">Yes</option>
		</select>
		</td>
	</tr>
    <tr>
		<td><?php echo JHTML::_('tooltip', "the time you want it to pause from the last keystroke before it validates (ms)" ); ?></td>
		<td class="key">Waiting time</td>
		<td></td>
		<td>
		<input type="text" name="params[validate_wait]" id="params[validate_wait]" class="inputbox" size="10" value="<?php echo $paramsvalues->get('validate_wait'); ?>">
		</td>
	</tr>
    <tr>
		<td><?php echo JHTML::_('tooltip', "whether should be validated only when the form it belongs to is submitted" ); ?></td>
		<td class="key">Run Validation onlyOnSubmit</td>
		<td></td>
		<td>
		<select name="params[validate_onlyOnSubmit]" id="params[validate_onlyOnSubmit]">
			<option<?php if($paramsvalues->get('validate_onlyOnSubmit') == '0'){ echo ' selected';} ?> value="0">No</option>
			<option<?php if($paramsvalues->get('validate_onlyOnSubmit') == '1'){ echo ' selected';} ?> value="1">Yes</option>
		</select>
		</td>
	</tr>
    <tr>
		<td><?php echo JHTML::_('tooltip', "Should the validation show the error messages or show your error divs with classes like : CF_LV_ERROR_field_id" ); ?></td>
		<td class="key">Validation Messages type</td>
		<td></td>
		<td>
		<select name="params[validation_type]" id="params[validation_type]">
			<option<?php if($paramsvalues->get('validation_type', 'default') == 'default'){ echo ' selected';} ?> value="default">default style</option>
			<option<?php if($paramsvalues->get('validation_type', 'default') == 'showalert'){ echo ' selected';} ?> value="showalert">Show My own Divs</option>
		</select>
		</td>
	</tr>
    <tr>
	...
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'ServerSide Validation' ); ?>
    <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::<?php echo JText::_( 'This is a Serverside validation which will work once the form is submitted, you are using PHP code here and any client side settings will not affect it so its the best way of validation if the data collected are critical!' ); ?>">
		<?php echo HTML_ChronoContact::WarningIcon(); ?>
    </span>
    </legend>
    <table class="admintable">
	...
	<div id="page-plugins">
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'Plugins Order' ); ?>
    <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::<?php echo JText::_( 'Pay attention: Some plugins work onsubmit the form event and will do some database changes, please assure that plugins are very well configured or this may end up saving wrong data to the database tables' ); ?>">
		<?php echo HTML_ChronoContact::WarningIcon(); ?>
    </span>
    </legend>
    <script type="text/javascript">
	...
	function enablePlugin(pluginname){
		if($('plugins_order_'+pluginname+'_enable_value').value != '1'){
			...
			$('plugins_order_'+pluginname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png';
		}else{
			$('plugins_order_'+pluginname).setStyle('background-color', '#DD0000');
			$('plugins_order_'+pluginname+'_enable_value').value = '0';
			$('plugins_order_'+pluginname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png';
		}
	}
	</script>
    <style type="text/css">
        ...
        <div id="plugins_container">        
            ...
            ?>
                <!--<tr>
                    ...
                    <td class="key"><?php echo ${$result}->result_TITLE; ?>:</td>
                    <td>
                ...
                ?>
                <div style="background-color:<?php echo $plugin_row_color; ?>" class="plugins_order" title="<?php echo ${$result}->result_TITLE; ?> :: <?php echo ${$result}->result_TOOLTIP; ?>" id="plugins_order_<?php echo ${$result}->plugin_name; ?>">
                    <span class="plugins_order_text"><?php echo ${$result}->result_TITLE; ?></span>
                    <input type="hidden" name="mplugins_order[]" id="plugins_order_<?php echo ${$result}->plugin_name; ?>_count_value" value="" />
                    <input type="hidden" name="plugins[]" value="<?php echo ${$result}->plugin_name; ?>" />
                    <input type="hidden" name="plugins_enable[]" id="plugins_order_<?php echo ${$result}->plugin_name; ?>_enable_value" value="<?php echo $plugin_enabled; ?>" />
                    <span class="plugins_order_enable" id="plugins_order_<?php echo ${$result}->plugin_name; ?>_enable"><img id="plugins_order_<?php echo ${$result}->plugin_name; ?>_enableimg" onclick="enablePlugin('<?php echo ${$result}->plugin_name; ?>')" src="<?php echo JURI::Base().'components/com_chronocontact/images/'.$plugin_state; ?>.png" width="16" height="16" alt="" /></span>
                    <span class="plugins_order_drag"><img src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>rearrange.png" width="16" height="16" alt="" /></span>
                    <span class="plugins_order_count" id="plugins_order_<?php echo ${$result}->plugin_name; ?>_count"></span>            
                </div>
        <?php
				if(in_array($result,explode(",", $paramsvalues->get('plugins')))){
					$order++;
	...
	<fieldset class="adminform">
    <legend><?php echo JText::_( 'RunOrder Settings' ); ?>
    <span class="error hasTip" title="<?php echo JText::_( 'Warning' );?>::<?php echo JText::_( 'Here you can change the order the different code blocks run, valid values are 1 or 2 or 3 using other values will result in the whole block doesn\'t executed!' ); ?>">
		<?php echo HTML_ChronoContact::WarningIcon(); ?>
    </span>
    </legend>
    <table class="admintable">
	<tr>
		...
		<td class="key"><?php echo "Order of Autogenerated block"; ?></td>
		<td></td>
		<td>
	...
	<tr>
		...
		<td class="key"><?php echo "Order of OnSubmit block"; ?></td>
		<td></td>
		<td>
	...
	<tr>
		...
		<td class="key"><?php echo "Order of Plugins block"; ?></td>
		<td></td>
		<td>
...
<!--</div>--><div class="clr"></div>
  ...
  echo JHTML::_('behavior.keepalive');
	...
	function maketableChronoContact( $row, $option, $table, $fields){
		global $mainframe;
		?>
		<?php echo JHTML::_('behavior.tooltip'); ?>
		<script type="text/javascript">
		window.addEvent('domready', function() {
			<?php if(!$table){ ?>
			new Sortables($('fields_container'), {		 
				initialize: function(){
					var step = 0;
					$ES('.fields_order_count', 'fields_container').each(function(element, i){
						step = step + 1;
						$(element.getProperty('id')+'_value').value = step;
					});
				},
				onComplete:	function(){
					var step2 = 0;
					$ES('.fields_order_count', 'fields_container').each(function(element){
						step2 = step2 + 1;
						$(element.getProperty('id')+'_value').value = step2;
					});
				},
				handles: 'span.fields_order_drag'
			});
			<?php } ?>
			var Tips1 = new Tips($$('.fields_order_drag'));
			var Tips2 = new Tips($$('.fields_order_enable'));
			var Tips3 = new Tips($$('.fields_order_enablekey'));
			var Tips4 = new Tips($$('.fields_order_enableincrement'));
			
		});
		function enableField(fieldname){
			if($('fields_order_'+fieldname+'_enable_value').value != '1'){
				$('fields_order_'+fieldname).setStyle('background-color', '#116600');
				$('fields_order_'+fieldname+'_enable_value').value = '1';
				$('fields_order_'+fieldname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png';
			}else{
				$('fields_order_'+fieldname).setStyle('background-color', '#FF3333');
				$('fields_order_'+fieldname+'_enable_value').value = '0';
				$('fields_order_'+fieldname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png';
			}
		}
		function enableallField(){
			if($('fields_order_enable_all').value != '1'){
				$ES('.fields_order', 'fields_container').each(function(element){
					element.setStyle('background-color', '#116600');
				});
				$ES('.fields_enable', 'fields_container').each(function(element){
					element.value = '1';
				});
				$ES('.fields_enable_img', 'fields_container').each(function(element){
					element.src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png';
				});
				$('fields_order_enable_all').value = '1';
				$('fields_order_all_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png';
			}else{
				$ES('.fields_order', 'fields_container').each(function(element){
					element.setStyle('background-color', '#FF3333');
				});
				$ES('.fields_enable', 'fields_container').each(function(element){
					element.value = '0';
				});
				$ES('.fields_enable_img', 'fields_container').each(function(element){
					element.src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png';
				});
				$('fields_order_enable_all').value = '0';
				$('fields_order_all_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png';
			}
		}
		function enableKey(fieldname){
			if($('iskey').value != fieldname){
				if($('iskey').value){
					$('fields_order_'+$('iskey').value+'_enablekeyimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>keyd.png';
				}
				$('iskey').value = fieldname;
				$('fields_order_'+fieldname+'_enablekeyimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>key.png';
			}else{
				$('iskey').value = "";
				$('fields_order_'+fieldname+'_enablekeyimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>keyd.png';
			}
		}
		function enableIncrement(fieldname){
			if($('isautoincrement').value != fieldname){
				if($('isautoincrement').value){
					$('fields_order_'+$('isautoincrement').value+'_enableincrementimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>incrementd.png';
				}
				$('isautoincrement').value = fieldname;
				$('fields_order_'+fieldname+'_enableincrementimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>increment.png';
			}else{
				$('isautoincrement').value = "";
				$('fields_order_'+fieldname+'_enableincrementimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>incrementd.png';
			}
		}
		var fieldcounter = 0;
		function addnewfield(){
			var divcontainer = new Element('div', {'class': 'fields_order', 'id':'fields_order_new'+fieldcounter}).setStyle('background-color', '#116600');
			var span_field_name = new Element('span', {'class': 'fields_order_text'});
			var span_fields_order = new Element('input', {'type': 'hidden', 'name':'fields_order[]', 'value':'', 'id':'fields_order_new'+fieldcounter+'_count_value'});
			var span_fields_enable = new Element('input', {'type': 'hidden', 'name':'fields_enable[]', 'class':'fields_enable', 'value':'1', 'id':'fields_order_new'+fieldcounter+'_enable_value'});
			
			var span_fields = new Element('input', {'type': 'text', 'name':'fields[]', 'value':'', 'class':'new_fields'});
			var span_fields_order_enable = new Element('span', {'class': 'fields_order_enable', 'id':'fields_order_new'+fieldcounter+'_enable'});
			var img_fields_order_enable = new Element('img', {'onclick': 'enableField("new'+fieldcounter+'")', 'class':'fields_enable_img', 'id':'fields_order_new'+fieldcounter+'_enableimg', 'src':'<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png', 'width':'16', 'height':'16'});
			
			var span_fields_order_enablekey = new Element('span', {'class': 'fields_order_enablekey', 'id':'fields_order_new'+fieldcounter+'_enablekey'});
			var img_fields_order_enablekey = new Element('img', {'onclick': 'enableKey("new'+fieldcounter+'")', 'id':'fields_order_new'+fieldcounter+'_enablekeyimg', 'src':'<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>keyd.png', 'width':'16', 'height':'16'});
			
			var span_fields_order_enableincrement = new Element('span', {'class': 'fields_order_enableincrement', 'id':'fields_order_new'+fieldcounter+'_enableincrement'});
			var img_fields_order_enableincrement = new Element('img', {'onclick': 'enableIncrement("new'+fieldcounter+'")', 'id':'fields_order_new'+fieldcounter+'_enableincrementimg', 'src':'<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>incrementd.png', 'width':'16', 'height':'16'});
						
			var span_fields_order_drag = new Element('span', {'class': 'fields_order_drag', 'id':'fields_order_new'+fieldcounter+'_drag'});
			var img_fields_order_drag = new Element('img', {'src':'<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>rearrange.png', 'width':'16', 'height':'16'});
			var span_fields_order_count = new Element('span', {'class': 'fields_order_count', 'id':'fields_order_new'+fieldcounter+'_count'});
			
			span_fields.injectInside(span_field_name);
			span_field_name.injectInside(divcontainer);
			span_fields_order.injectInside(divcontainer);
			span_fields_enable.injectInside(divcontainer);
			
			img_fields_order_enable.injectInside(span_fields_order_enable);
			span_fields_order_enable.injectInside(divcontainer);
			img_fields_order_enablekey.injectInside(span_fields_order_enablekey);
			span_fields_order_enablekey.injectInside(divcontainer);
			img_fields_order_enableincrement.injectInside(span_fields_order_enableincrement);
			span_fields_order_enableincrement.injectInside(divcontainer);
			<?php if(!$table){ ?>
			img_fields_order_drag.injectInside(span_fields_order_drag);
			span_fields_order_drag.injectInside(divcontainer);
			<?php } ?>
			var newtype = $('temp_type').clone();
			newtype.setProperty('name', 'fields_types[]');
			newtype.setProperty('id', 'fields_new'+fieldcounter+'_type');
			newtype.setStyle('display', 'inline');
			newtype.injectInside(span_fields_order_count);
			span_fields_order_count.injectInside(divcontainer);
			divcontainer.injectInside($('fields_container'));
		...
		function savetable(){
			var patt1 = new RegExp(/^[a-zA-Z][a-zA-Z_0-9]+$/);
			var newfieldsok = true;
			$ES('.new_fields', 'fields_container').each(function(element, i){
				if(element.value.trim() && !patt1.test(element.value.trim()) && newfieldsok){
					newfieldsok = false;
				}
			});
			if(!patt1.test($('istable_name').value.trim())){
				alert('Enter a valid table name with alphanumeric characters or underscore');
				return false;
			}else if(!newfieldsok){
				alert('Enter a valid field name with alphanumeric characters or underscore');
				return false;
			}else{
				$('adminForm').submit();
			}
		...
		span.table_name_enable{
			float:right;
			font-size:14px;
			font-weight:bold;
			padding:0px 0px 0px 10px;
		}
		span.table_name_text{
			float:left;
			font-size:12px;
			font-weight:bold;
			color:#000000;
		...
		span.fields_order_count{
			float:right;
			font-size:14px;
			font-weight:bold;
		}
		
		span.fields_order_enable{
			float:right;
			font-size:14px;
			font-weight:bold;
			padding:0px 0px 0px 10px;
		...
		span.fields_order_enableincrement{
			float:right;
			font-size:14px;
			font-weight:bold;
			padding:0px 0px 0px 10px;
	  ...
	  <form action="index2.php" method="post" name="adminForm" id="adminForm">       
        <img src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>add.png" style="cursor: pointer;" onClick='addnewfield();' width="32" height="32" alt="" />
        <img src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>save.png" style="cursor: pointer;" onClick='savetable();' width="32" height="32" alt="" />
  		<div class="table_name">
            <span class="table_name_text">Table Name:</span>
            <span class="table_name_enable">
            <input type="text" name="istable_name" id="istable_name" size="60" value="<?php echo ($table) ? $table : $mainframe->getCfg('dbprefix')."chronoforms_".$row->name; ?>" />
            </span>
        </div>
        <input type="hidden" name="option" value="<?php echo $option; ?>" />
        <input type="hidden" name="formid" value="<?php echo $row->id; ?>" />
        <input type="hidden" name="tableexists" value="<?php echo $table; ?>" />
        ...
        <div style="background-color:#0099FF" class="fields_order_all">
        <span class="fields_order_text">Field name</span>
        <span class="fields_order_enable" id="fields_order_all_enable"><img id="fields_order_all_enableimg" onclick="enableallField()" src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png" width="16" height="16" alt="" /></span>
        <input type="hidden" name="fields_order_enable_all" id="fields_order_enable_all" value="0" />
        </div>
	...
	<?php
		...
		$iskey = 'cf_id';
		$isautoincrement = 'cf_id';
		foreach($fields as $field => $fielddata){
			?>
            <?php 
                if(in_array($field, $defaults)){
                    $field_state = 'disable';
                    $field_row_color = "#116600";
                    $field_enabled = "1";
					$key_state = 'keyd';
					$increment_state = 'incrementd';
					$field_enabledkey = '0';
					$field_enabledincrement = '0';
                    ...
                    $field_state = 'enable';
                    $field_row_color = "#FF3333";
                    $field_enabled = "0";
					$key_state = 'keyd';
					$increment_state = 'incrementd';
                }
            ?>
            <div style="background-color:<?php echo $field_row_color; ?>" class="fields_order" id="fields_order_<?php echo $field; ?>">
                <span class="fields_order_text"><?php echo $field; ?></span>
                <input type="hidden" name="fields_order[]" id="fields_order_<?php echo $field; ?>_count_value" value="" />
                <input type="hidden" name="fields[]" value="<?php echo $field; ?>" />
                <input type="hidden" class="fields_enable" name="fields_enable[]" id="fields_order_<?php echo $field; ?>_enable_value" value="<?php echo $field_enabled; ?>" />
                <span class="fields_order_enable" title="<?php echo $field; ?> :: Enable/Disable" id="fields_order_<?php echo $field; ?>_enable"><img class="fields_enable_img" id="fields_order_<?php echo $field; ?>_enableimg" onclick="enableField('<?php echo $field; ?>')" src="<?php echo JURI::Base().'components/com_chronocontact/images/'.$field_state; ?>.png" width="16" height="16" alt="" /></span>
                <span class="fields_order_enablekey" title="<?php echo $field; ?> :: Enable Primary key" id="fields_order_<?php echo $field; ?>_enablekey"><img id="fields_order_<?php echo $field; ?>_enablekeyimg" onclick="enableKey('<?php echo $field; ?>')" src="<?php echo JURI::Base().'components/com_chronocontact/images/'.$key_state; ?>.png" width="16" height="16" alt="" /></span>
                <span class="fields_order_enableincrement" title="<?php echo $field; ?> :: Enable Auto increment" id="fields_order_<?php echo $field; ?>_enableincrement"><img id="fields_order_<?php echo $field; ?>_enableincrementimg" onclick="enableIncrement('<?php echo $field; ?>')" src="<?php echo JURI::Base().'components/com_chronocontact/images/'.$increment_state; ?>.png" width="16" height="16" alt="" /></span>
                <?php if(!$table){ ?>
                <span class="fields_order_drag" title="<?php echo $field; ?> :: Drag to change order" id="fields_order_<?php echo $field; ?>_drag"><img src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>rearrange.png" width="16" height="16" alt="" /></span>
                <?php } ?>
                <span class="fields_order_count" id="fields_order_<?php echo $field; ?>_count">
                <?php if(!$fielddata->Type){ ?>
                	<select name="fields_types[]" id="fields_<?php echo $field; ?>_type">
                        <option value="VARCHAR(255)">VARCHAR(255)</option>
                        <option value="TINYINT">TINYINT</option>
                        <option value="TEXT">TEXT</option>
                        <option value="DATE">DATE</option>
                ...
                <?php }else{ ?>
                <?php echo $fielddata->Type; ?> <input type="hidden" name="fields_types[]" value="<?php echo $fielddata->Type; ?>" />
                <?php } ?>
                </span>            
            </div>
            <?php
        ...
        <input type="hidden" name="iskey" id="iskey" value="<?php echo $iskey; ?>">
        <input type="hidden" name="isoldkey" id="isoldkey" value="<?php echo $iskey; ?>">
        <input type="hidden" name="isautoincrement" id="isautoincrement" value="<?php echo $isautoincrement; ?>">
        </form>
        <?php
		echo JHTML::_('behavior.keepalive');
	}

	function tablemanagerChronoContact($option, $results){
		global $mainframe;
		?>
        <?php JHTML::_('behavior.modal'); ?>
        	<script type="text/javascript">
				window.addEvent('domready', function() {
					var Tips1 = new Tips($$('.tables_order_enable'));
					var Tips2 = new Tips($$('.tables_order_edit'));
					var Tips3 = new Tips($$('.tables_order_view'));					
				});
				function enableTable(fieldname){
					if($('tables_order_'+fieldname+'_enable_value').value != '1'){
						$('tables_order_'+fieldname).setStyle('background-color', '#CCCCCC');
						$('tables_order_'+fieldname+'_enable_value').value = '1';
						$('tables_order_'+fieldname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png';
					}else{
						$('tables_order_'+fieldname).setStyle('background-color', '#FF3333');
						$('tables_order_'+fieldname+'_enable_value').value = '0';
						$('tables_order_'+fieldname+'_enableimg').src = '<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>enable.png';
					}
				}
				function applyTableChanges(){
					var dropexists = false;
					$ES('.tables_enable', 'adminForm').each(function(element, i){
						if(element.value == 0){
							dropexists = true;
						}
					});
					if(dropexists){
						var content = 'Some tables will be dropped, continue ?<br /><br />';
						content = content + '<form action="index2.php" method="post" name="adminForm" id="adminForm"><input type="submit" value="submit" name="submitbutton" />';
						var formcontent = $('tables_container').clone();
						$ES('.table_name', formcontent).each(function(element, i){
							element.setStyle('display', 'none');
						});
						content = content + formcontent.innerHTML;
						content = content + '</form>';
						SqueezeBox.initialize();
						SqueezeBox.setContent('iframe', '');
						SqueezeBox.applyContent('<div style="color:#FF0000; padding:5px; font-size:12px;">'+content+'</div>', {x: 200, y: 100});
			...
			span.tables_order_enable{
				float:right;
				font-size:14px;
				font-weight:bold;
				padding:0px 0px 0px 10px;
			}
			span.tables_order_edit{
				float:right;
				font-size:14px;
				font-weight:bold;
				padding:0px 0px 0px 10px;
			}
			span.tables_order_view{
				float:right;
				font-size:14px;
				font-weight:bold;
				padding:0px 0px 0px 10px;
        ...
        <form action="index2.php" method="post" name="adminForm" id="adminForm">
        <img src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>apply.png" style="cursor: pointer;" onClick='applyTableChanges();' width="32" height="32" alt="" />
        <div id="tables_container">
		<?php
		foreach($results as $k => $table){
		?>
        	
        	<div class="table_name" id="tables_order_<?php echo $k; ?>">
                <span class="table_name_text"><?php echo $table; ?></span>
                <input type="hidden" name="tables[]" value="<?php echo $table; ?>" />
                <input type="hidden" class="tables_enable" name="tables_enable[]" id="tables_order_<?php echo $k; ?>_enable_value" value="1" />
                <span class="tables_order_enable" title="<?php echo $table; ?> :: Enable/Disable" id="tables_order_<?php echo $k; ?>_enable"><img class="tables_enable_img" id="tables_order_<?php echo $k; ?>_enableimg" onclick="enableTable('<?php echo $k; ?>')" src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>disable.png" width="16" height="16" alt="" /></span>
                <span class="tables_order_edit" title="<?php echo $table; ?> :: Edit table" id="tables_order_<?php echo $k; ?>_edit"><a href="index.php?option=com_chronocontact&table=<?php echo $table; ?>&task=edittable"><img id="tables_order_<?php echo $k; ?>_editimg" src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>edit.png" width="16" height="16" alt="" /></a></span>
                <span class="tables_order_view" title="<?php echo $table; ?> :: View table" id="tables_order_<?php echo $k; ?>_view"><a href="index.php?option=com_chronocontact&table=<?php echo $table; ?>&task=show"><img id="tables_order_<?php echo $k; ?>_viewimg" src="<?php echo JURI::Base().'components/com_chronocontact/images/'; ?>view.png" width="16" height="16" alt="" /></a></span>
            </div>
        <?php
		}
		?>
        <input type="hidden" name="option" value="<?php echo $option; ?>" />
        <input type="hidden" name="task" value="updatetablelist" />
        </div>
        </form>
        <?php
	}
...
function showdataChronoContact( $rows, $pageNav, $option, $formid, $table ) {
    ...
    <tr>
        ...
        <th class='title'><?php echo str_replace(array("{", "}"), array("", ""), $paramsvalues->get('dvrecord', 'Record #n')); ?></th>        
    ...
    for ($i=0, $n=count($rows); $i < $n; $i++) {
	  ...
	  <tr class="<?php echo "row$k"; ?>">
		...
		<?php 
		$dvrecord = $paramsvalues->get('dvrecord', 'Record #n');
    ...
    <form action="index2.php" method="post" name="adminForm" enctype="multipart/form-data">
        <table cellpadding="0" cellspacing="0" border="0" width="100%" class="adminlist">
        <tr><td>Choose your .cfbak/.cfxbak form file: &nbsp;<input type="file" name="file"></td></tr>
	...
	global $mainframe;
		...
		<!--[if gte IE 6]><link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>style1-ie6.css" rel="stylesheet" type="text/css" /><![endif]-->
		<!--[if gte IE 7]><link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>style1-ie7.css" rel="stylesheet" type="text/css" /><![endif]-->
		<!--[if IE 8]> <--><link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>style1.css" rel="stylesheet" type="text/css" /><!--> <![endif]-->
		<!--[if !IE]> <--><link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>style1.css" rel="stylesheet" type="text/css" /><!--> <![endif]-->
		<link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>calendar.css" rel="stylesheet" type="text/css" />
		<link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>tooltip.css" rel="stylesheet" type="text/css" />
		<link href="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>smoothbox/smoothbox.css" rel="stylesheet" type="text/css" />
...
<form action="index2.php" method="post" name="adminForm">
			...
			<!--  end of addon -->
				...
				<div id="right_column">
					...
					<div class="items">
                        ...
                        <?php					
							$database =& JFactory::getDBO();
							$query = "SELECT * FROM #__chrono_contact_elements ORDER BY id";
							$database->setQuery( $query );
							$wizardelements = $database->loadObjectList();
						?>
                        <?php foreach($wizardelements as $wizardelement) { ?>
							<?php if($wizardelement->title){ ?>
                            <div class="item"><span id="cf_placeholder" number="<?php echo $wizardelement->id; ?>" holder="<?php echo $wizardelement->placeholder; ?>" params="<?php echo str_replace("\n", "*,*", $wizardelement->params); ?>"><?php echo $wizardelement->title; ?></span></div>
                            <?php } ?>
                        <?php } ?>
					</div>
						...
						<div class="box_header border-top">Properties</div>        
							<div class="box_text">
								...
								<div id="prop_cf_dropdown" class="Propertiesitem" style="display:none ">
									...
									<hr />
                                    <span>Add Choose Option:</span> <input name="prop_cf_dropdown_firstoption" type="checkbox" id="prop_cf_dropdown_firstoption" value="" />
									<hr />
                                    <span>Choose Option Text:</span> <input name="prop_cf_dropdown_firstoptiontext" id="prop_cf_dropdown_firstoptiontext" type="text" value="" size="10">
									<hr />
                                ...
                                <?php foreach($wizardelements as $wizardelement) { ?>
                                <div id="prop_cf_placeholder_<?php echo $wizardelement->id; ?>" class="Propertiesitem" style="display:none ">
									<?php
									$wizardelementparams = explode("\n", $wizardelement->params);
                                    foreach($wizardelementparams as $wizardelementparam){
										if(trim($wizardelementparam)){
											$paramdata = explode('=', $wizardelementparam);
											?>
											<tr>
                                                <span><?php echo $paramdata[0]; ?>:</span> <input name="prop_cf_placeholder_<?php echo $wizardelement->id; ?>_<?php echo $paramdata[0]; ?>" type="text" id="prop_cf_placeholder_<?php echo $wizardelement->id; ?>_<?php echo $paramdata[0]; ?>" value="<?php //echo $paramdata[1]; ?>" size="10" />
                                                <hr />
											</tr>
											<?php 
										}
                                    ...
                                    ?>									
									<input id="prop_cf_placeholder_<?php echo $wizardelement->id; ?>_done" type="button" name="prop_cf_placeholder_<?php echo $wizardelement->id; ?>_done" value="Apply" class="cf_button1" />
								</div>
                                <?php } ?>
			...
			<div id="container">				
				<div class="float_left width1" >
					<div id="top_column4">
						...
						<?php if(!isset($noeditor)){ ?><a href="#" onClick="ShowAddField2();" name="Add Field"><img id="cf_insertfieldname2" title="Add Field :: Choose some field to insert its value at the choosed position in the after submit text template" border="0" src="<?php echo JURI::Base().'components/com_chronocontact/css/'; ?>images/note_f2.png" width="32" height="32"></a><?php } ?>
	...
	function wizard_elements( $rows, $pageNav, $option ){
		?>
         <form action="index2.php" method="post" name="adminForm">
            ...
            <table class="adminheading">
            <tr>
            <th>ChronoForms - Wizard's Custom Elements Manager</th>
            </tr>
            </table>
	...
	function editElement( $row, $option ){
		...
		function addparamm(){
			...
			if(!patt1.test($('newparam').value.trim())){
				alert('Enter a valid parameter name with alphanumeric characters or underscore');
				return false;
			}
			tablerow = new Element('tr');
			tablecell1 = new Element('td');
			tablecell2 = new Element('td');
			div1 = new Element('div').setHTML('Param name: <strong>'+$('newparam').value+'</strong>');
			div2 = new Element('div').setText('Default value if any: ');
			text2 = new Element('input', {'name':'params['+$('newparam').value+']', 'value':'', "type":'text'});
			tablerow.injectInside($('paramstable'));
			tablecell1.injectInside(tablerow);
			tablecell2.injectInside(tablerow);
			div1.injectInside(tablecell1);
			div2.injectInside(tablecell2);
			text2.injectInside(tablecell2);
			$('newparam').value = '';
		}
		</script>
         <form action="index2.php" method="post" name="adminForm">
        	<table class="adminlist" id="paramstable">
            	<tr>
                	<td>Wizard Title</td>
				...
				?>
                    <tr>
                        <td><div>Param name: <strong><?php echo $paramdata[0]; ?></strong></div></td>
                        <td><div>Default value if any: </div><input type="text" value="<?php echo $paramdata[1]; ?>" name="params[<?php echo $paramdata[0]; ?>]"/></td>
                    </tr>
                <?php 
					}
				}
				?>
                <tr>
                	<td><input type="button" name="addparam" id="addparam" value="Insert New Parameter" onclick="addparamm();" /></td>
                    <td><input type="text" name="newparam" id="newparam" value="" /></td>
                </tr>
            </table>
        	<input type="hidden" name="option" value="<?php echo $option; ?>" />
	...
	function showLicenseStatus(){
		...
		if($configs->get('licensevalid', '')){
            ...
            <?php
				...
				//print_r($matches);
			?>
            <span class="cf_alert">Your Chronoforms install at <?php echo $matches[2]; ?> is <strong>Validated</strong></span>
            <?php
		}else{
			?>
            <style type="text/css">
				span.cf_alert {
					background:#FFD5D5 url(<?php echo $mainframe->getSiteURL().'components/com_chronocontact/css/'; ?>images/alert.png) no-repeat scroll 10px 50%;
            ...
            <?php
				...
				//print_r($matches);
			?>
            <span class="cf_alert">Your Chronoforms install at <?php echo $matches[2]; ?> is <strong>NOT Validated</strong>, No limited features but for a small fee you get link free forms and help us continue the development and support</span>
            <?php
		}
	}	
	function validatelicensepage( $option ){
		global $mainframe;
		preg_match('/http(s)*:\/\/(.*?)\//i', $mainframe->getSiteURL(), $matches);
		$database =& JFactory::getDBO();
		$query     = "SELECT * FROM `#__components` WHERE `option` = 'com_chronocontact' AND parent='0' AND admin_menu_link='option=com_chronocontact'";
		...
		?>
        <font style="size:16px; font-weight:bold">We are about to validate the details below, please double check that everything is correct and hit "Validate"</font><br /><br />
        ...
        <table class="adminlist">
                ...
                <tr>
					...
					if(!$configs->get('licensecode', '')){ ?>
						<font style="color:#FF0000; font-weight:bold;">Please add your key code to the parameters box in the forms management area first!</font>
                    <?php
					}else{
                    	echo $configs->get('licensecode', '');
					}
					?>
                    </td>
                </tr>
                ...
                <tr>
                	<td>Instant Code:</td>
                    <td><input type="text" name="instantcode" size="150" value="" /></td>
                </tr>
            </table>
            <input type="submit" value="Validate" />
        	<input type="hidden" name="option" value="<?php echo $option; ?>" />
            <input type="hidden" name="task" value="validatelicensedata" />
        </form>
        <?php
	}
