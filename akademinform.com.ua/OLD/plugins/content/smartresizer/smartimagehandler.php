...
define("smccTOPLEFT", 0);
define("smccTOP", 1);
define("smccTOPRIGHT", 2);
define("smccLEFT", 3);
define("smccCENTRE", 4);
define("smccCENTER", 4);
define("smccRIGHT", 5);
define("smccBOTTOMLEFT", 6);
define("smccBOTTOM", 7);
define("smccBOTTOMRIGHT", 8);

class smartcanvasCrop{
    var $_imgOrig;
    var $_imgFinal;
    var $_showDebug;
    var $_gdVersion;
    ...
    */
    function smartcanvasCrop($debug = false)
    {
    ...
    */
    function cropBySize($x, $y, $position = smccCENTRE)
    {
        if ($x == 0){
    ...
    */
    function cropToSize($x, $y, $position = smccCENTRE)
    {
        if ($x == 0) $x = 1;
        if ($y == 0) $y = 1;
    ...
    */
    function cropByPercent($percentx, $percenty, $position = smccCENTRE)
    {
        if ($percentx == 0){
            $nx = @ImageSX($this->_imgOrig);
        }else{
    ...
    */
    function cropToPercent($percentx, $percenty, $position = smccCENTRE)
    {
        if ($percentx == 0){
            $nx = @ImageSX($this->_imgOrig);
        }else{
    ...
    */
    function cropByAuto($threshold = 254)
    {
        ...
        $nx = abs($ex - $sx);
        $ny = abs($ey - $sy);
        return ($this->_cropSize($sx, $sy, $nx, $ny, smccTOPLEFT, 'cropByAuto'));
    }
...
class smartimgRedim extends smartcanvasCrop{
    var $temp_directory;
    var $improve_thumbs;

    function smartimgRedim($debug = false, $filters = false, $temp_path)
    {
        $this->smartcanvasCrop($debug);
        if (empty($temp_path)){
