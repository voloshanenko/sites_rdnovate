...
class JoomSEF
{
    ...
    function _parseSefUrl(&$uri, &$oldUri)
    {
        ...
        else
        {
            ...
            if ($sefConfig->record404) {
                $query = "SELECT id FROM `#__sefurls` WHERE `sefurl` = '{$oldRoute}' AND `trashed` = '0'";
                $db->setQuery($query);
                ...
                if ($resultId) {
                    ...
                    . " VALUES ( '1', '$oldRoute', '', {$traceinfo}, CURDATE() )";
                    $db->setQuery($query);
                    $db->query();
                }
            }
    ...
    function _getMenuItemInfo($option, $task, $id = null, $string = null)
    {
        ...
        if (!empty($row)) {
            ...
            if (!isset($string) && !isset($id)) {
                // Try to extend the search for any link to component
                $sql = "SELECT `$column` AS `name`, `params`$jfTranslate FROM `#__menu` WHERE `link` LIKE 'index.php?option=$option%' AND `published` > 0";
                $db->setQuery($sql);
                $row = $db->loadObject();
                if (!empty($row)) {
                    if (!empty($row->name) && !$item->title) $item->title = $row->name;
    ...
    function _storeLocation(&$data)
    {
        ...
        if (!$sefConfig->useCache || !$realloc) {
            $query = "SELECT * FROM `#__sefurls` WHERE `origurl` = '" . addslashes(html_entity_decode(urldecode($url))) . "'" . $where . " AND (`trashed` = '0') LIMIT 2";
            $db->setQuery($query);
        ...
        // URL not found, so lets create it
            ...
            $sql = "SELECT `id`, `origurl`, `Itemid`, `sefurl` FROM `#__sefurls` WHERE `sefurl` = '{$temploc}' AND `origurl` != '' AND `trashed` = '0'";
            $db->setQuery($sql);
            $row = $db->loadObject();
            ...
            if( is_null($realloc) ) {
                ...
                $sql = "SELECT `id`, `origurl`, `Itemid`, `sefurl` FROM `#__sefurls` WHERE `sefurl` LIKE '{$leftPart}%{$rightPart}' AND `trashed` = '0'";
                $db->setQuery($sql);
    ...
    function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url, $check404 = false)
    {
        ...
        if (!is_null($row)) {
            ...
            $query = "UPDATE `#__sefurls` SET `sefurl` = '{$temploc}', `trashed` = '0' WHERE `id` = '{$row->id}' LIMIT 1";
            $db->setQuery($query);
            if (!$db->query()) {
            ...
            $db->setQuery("DELETE FROM `#__sefurls` WHERE `sefurl` = '{$temploc}' AND `origurl` = '' LIMIT 1");
            if (!$db->query()) {
                $msg = JText::_('Could not update the SEF URL in database');
                if (JDEBUG) {
                    $msg .= ': '.$db->getErrorMsg();
        ...
        "VALUES ('".$temploc."', ".$db->Quote(html_entity_decode(urldecode($url))).", '$priority'" . $val . $metavals . ", " . $traceinfo . ", '{$sm_indexed}', '{$sm_date}', '{$sm_frequency}', '{$sm_priority}')";
        $db->setQuery($query);
