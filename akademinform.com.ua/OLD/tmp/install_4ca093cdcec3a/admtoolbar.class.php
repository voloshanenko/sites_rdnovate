<?php
/**
* @version $Id: admtoolbar.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobiAdmToolbar {
	...
	*/
	...
	function checkImage( $icon, $alt ) {
		if(sobi2_eregi("msie",$_SERVER['HTTP_USER_AGENT']) && !sobi2_eregi("opera",$_SERVER['HTTP_USER_AGENT'])) {
	...
	*/
	function help( $ref, $com=false,  $alt = null ) {
	   ...
	   // help file for 3PD Components
			...
			if (!sobi2_eregi( '\.html$', $ref ) && !sobi2_eregi( '\.xml$', $ref )) {
				$ref = $ref . '.html';
			}
			$url .= $ref;
		} else if ( $helpUrl ) {
