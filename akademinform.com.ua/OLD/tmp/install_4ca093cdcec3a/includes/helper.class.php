...
class sobi2Helper {
	...
	function meta( $desc, $keys, $cid = 1 )
	{
		...
		if( count( $meta[ $cid ] ) ) {
			...
			if( $cid > 1 ) {
				$metaKeys .= $config->getSobiStr( $config->componentName.', '.$config->getSobiStr( $comMeta->name ).', '.$config->getSobiStr( $comMeta->introtext ) );
			}
			else {
				$metaKeys .= $config->getSobiStr( $config->componentName );
			}
		}
