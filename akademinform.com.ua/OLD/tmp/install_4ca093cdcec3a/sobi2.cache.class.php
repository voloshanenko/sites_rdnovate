<?php
/**
 * @version $Id: sobi2.cache.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobiCache
{
	...
	function addContent ( $content, $params, $section )
	{
		...
		if ( $strlen > $config->cacheL2strLen ) {
			trigger_error( "L2 cache value too big to save in database. Allowed L2 size is: {$config->cacheL2strLen}. Cache value size is: {$strlen}. Skipping for S:{$section}/C:{$cid}/T:{$task}/U:{$uid}", E_USER_WARNING );
			return false;
		}
	...
	function getContent ( $params, $section )
	{
		...
		if ( $result ) {
			if ( strlen( $result->html ) != $result->schecksum ) {
				trigger_error( "Cached content size does not match checksum. Cached content removed. S:{$sid},T:{$task},C:{$cid},U:{$uid},S:{$section}", E_USER_WARNING );
	...
	function addObj ( $obj, $sid = 0, $cl = 0, $cid = 0, $oid = 0, $force = false )
	{
		...
		if ( $strlen > $config->cacheL3strLen ) {
			trigger_error( "L3 cache value too big to save in database. Allowed L3 size is: {$config->cacheL3strLen}. Cache value size is: {$strlen}. Skipping for C:{$cid}/S:{$sid}/CL:{$cl}", E_USER_WARNING );
			return false;
		}
	...
	function getObj ( $sid = 0, $cl = 0, $cid = 0, $oid = 0, $ignoreLang = false, $force = false )
	{
		...
		if ( $var && is_object( $var ) ) {
			...
			if ( $var->schecksum != $size ) {
				trigger_error( "Cached object size does not match checksum. Cached object removed. S:{$sid}, CL:{$cl}, C:{$cid}, O:{$oid} ", E_USER_WARNING );
	...
	function recountCats ( $cats )
	{
		...
		foreach( $cids as $cid ) {
			...
			$query = "SELECT COUNT({$distinct}rel.itemid) FROM `#__sobi2_cat_items_relations` AS rel " . "LEFT JOIN `#__sobi2_item` AS sitem ON rel.itemid = sitem.itemid " . "WHERE(sitem.published = '1' AND (sitem.publish_down > '{$now}' OR sitem.publish_down = '{$config->nullDate}') AND rel.catid IN ({$childs}))";
			$db->setQuery( $query );
			$items = $db->loadResult();
			if ( $db->getErrorNum() ) {
				trigger_error( "DB reports: " . $db->stderr(), E_USER_WARNING );
