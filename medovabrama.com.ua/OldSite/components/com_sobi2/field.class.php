<?php
/**
* @version $Id: field.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobiField {
	...
	function sobiField($id, $itemid = null)
	{
		...
		if(!$itemid) {
			...
			if($field) {
				...
				$this->explanation = $config->getSobiStr($field->explanation);
		...
		if( ( ( ($user->id && $user->id == $sobi->owner && $config->allowUserToEditEntry ) || $config->checkPerm() ) && $config->allowQuickEdit == 1 ) || ( $config->checkPerm() && $config->allowQuickEdit == 2 ) ) {
			...
			if( !$value && $field->fieldType != 6 ) {
				...
				if($fieldData->with_label) {
					...
					echo "<div id=\"sobi2Listing_{$field->fieldname}_label\">{$field->label}:</div>";
				}
				echo $value;
				exit();
			}
	...
	function showMediaData($itemid = 0, $fid, $data = 0, $w = 0, $h = 0, $autostart = 0, $cssId = 0) {
		...
		if ($position) {
			$fileExt = strtolower(substr($data,$position+1));
		}
		...
		else if(in_array($fileExt,$config->allowedExtVideo))
		{
			...
			/* we have to find the first / and compare it with the allowed address.
