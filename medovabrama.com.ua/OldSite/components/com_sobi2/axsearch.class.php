<?php
/**
* @version $Id: axsearch.class.php 5472 2010-09-23 13:07:52Z Sigrid Suski $
...
class sobiAxSearch {
	function search()
	{
		...
		foreach( $fields as $field ) {
			...
			$config->sobi2setcookie("sobi2SearchCookie[{$field->fieldname}]", $getFieldCookie);
		...
		$config->sobi2setcookie("sobi2SearchCookie[cid]", $cid);
		...
		if( $total ) {
			...
			$config->sobi2setcookie("sobi2SearchCookie[SobiSearchPage]", $limitstart);
		...
		$config->sobi2setcookie("sobi2SearchCookie[sobi2Search]", $searchString);
		$config->sobi2setcookie("sobi2SearchCookie[searchphrase]", $phrase);
		...
		if( count( $selectedCats ) ) {
			...
			foreach( $selectedCats as $cid ) {
				...
				if( is_array( $cats ) && !empty( $cats ) ) {
					...
					$Select = array();
					$Select[] = sobiHTML::makeOption( 0, '- оберiть -');
		 ...
		 /* <![CDATA[ */
			...
			function SobiSearchFormCatGetSubcats(XMLDoc,c) {
				...
				if(categories.length > 0) {
					...
					html = "";
					html = html + "<div id='sdrops_"+SobiSearchFormComboBxCounter+"'><select class='inputbox catChooseBox' id='SobiCatSelected_" + SobiSearchFormComboBxCounter + "' onchange='addSobiSearchFormCatBox(this.options[this.selectedIndex].value," + SobiSearchFormComboBxCounter + ");'><option value='0'><?php echo _SOBI2_SEARCH_CATBOX_SELECT;?></option>"
