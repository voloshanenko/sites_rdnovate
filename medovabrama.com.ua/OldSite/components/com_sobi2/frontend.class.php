...
class frontend {
	...
	function buildHeader($catid = 0, $params, $sobi2Task) {
		...
		$this->sobi2Header = "<div class='sobi2'>\n";
						...
						if (strlen ( $data ) > 0) {
							...
							if ($field->with_label) {
								if ($field->fieldType != 6) {
									$data = "<{$tag} class=\"sobi2Listing_{$field->name}\"><span class=\"sobi2Listing_{$field->name}_label\">{$field->label}:</span><div class=\"field_data\">{$data}</div></{$tag}>";
								} else {
									$data = "<span class=\"sobi2Listing_{$field->name}_label\">{$field->label}:</span> {$data}";
								}
							} else {
								if ($field->fieldType != 6) {
									$data = "<{$tag} class=\"sobi2Listing_{$field->name}\"><div class=\"field_data\">{$data}</div></{$tag}>";
								}
							}
							if ($field->in_newline) {
								$data = "<br/>" . $data;
