<?php
/**
* @version $Id: admin.config.class.html.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
 ...
 */
defined ('SOBI_CODEEDITORPATH') || define ('SOBI_CODEEDITORPATH',"/administrator/components/com_sobi2/includes/codemirror/");
		      ...
		      <td valign="top">
					  <table class="SobiAdminForm" border = 1 width="100%">
					    <tr>
					      ...
					      <td valign="top" width="20%"><?php echo sobi2AdmHelper::amImages( 'image', $config->componentDesc->image, null, $config->catImagesFolder ); ?></td>
				...
				<?php
				 ...
				 ?>
					<script type="text/javascript">
						...
						document.write('<img src=' + bsimg + ' name="backgroundimage" width="50" height="50" style="border-color:#<?php echo $config->sobi2BorderColor;?>; border-width:2px; border-style:solid;" alt="Preview" />');
					</script>
			  </td>
		    </tr>
		</table>
...
?>
		  <table class="SobiAdminForm" width="100%">
		    ...
		    <tr class="row1">
		      ...
		      <td valign="top" width="80%">
			  <?php $config->bankData = str_replace(array( '<br />', '\r', '\n' ), array( null, "\r", "\n" ), $config->bankData); ?>
		      <?php $config->bankData = stripslashes(stripslashes($config->bankData)); ?>
				  <textarea class="text_area" name="bankData" rows="10" cols="50" wrap="off"><?php echo $config->bankData; ?></textarea>
			  </td>
		    </tr>
		    <tr class="row0">
	...
	function editCSS( &$config )
	{
		...
		$config->mainframe->addCustomHeadTag("<script src=\"{$config->liveSite}".SOBI_CODEEDITORPATH."/js/codemirror.js\" type=\"text/javascript\"></script>");
		if(defined("_SOBI_MAMBO")) {
			echo "<script src=\"{$config->liveSite}".SOBI_CODEEDITORPATH."/js/codemirror.js\" type=\"text/javascript\"></script>";
		}
		?>
		
		<table class="SobiAdminForm">
		<tr><th><?php echo _SOBI2_MENU_EDIT_CSS; ?></th></tr>
		<tr><td>
		...
		<tr><td>
			<textarea style="width:100%" cols="110" rows="45" name="csscontent" id="csscontent"><?php echo $open; ?></textarea>
		</td></tr>
		</table>

		<script type="text/javascript">
		function submitbutton() {
			$('csscontent').value = editor.getCode();
			$each( $ES( 'input[type=hidden]', 'adminForm' ), function( el ) { if( el.name == 'task' ) el.value = 'saveConfig' } );
			$('adminForm').submit();
		}
		var editor = CodeMirror.fromTextArea('csscontent', {
		   height: "350px",
		   parserfile: ["parsecss.js"],
		   stylesheet: ["<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>css/csscolors.css"],
		   path: "<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>js/",
	...
	?>
		<table class="SobiAdminForm" width="100%">
			<tr>
				<th><?php echo _SOBI2_MENU_EDIT_TEMPLATE ?></th>
			</tr>
			<tr class="row0">
				<td><span class="editlinktip"><?php echo _SOBI2_CHOOSE_TPL_TO_EDIT; ?></span>&nbsp;
				<?php echo sobi2AdmHelper::templatesChooser( $tpl, 'stpl', 'details', array( 'onchange' => 'window.location=window.location+\'&stpl=\'+this.options[this.selectedIndex].value '), false ); ?></td>
			</tr>
			<tr class="row1">
				<td>... <?php echo str_replace( _SOBI_CMSROOT, null, $templatefile ) ; echo _SOBI2_GENERAL_FILE_IS; ?>
	 			<?php echo is_writable($templatefile) ? _SOBI2_GENERAL_FILE_WRITABLE : _SOBI2_GENERAL_FILE_UNWRITABLE; ?></td>
	 		</tr>
		</table>
		<?php
	...
	<tr><td>
		<textarea style="width:100%" cols="110" rows="45" name="templateContent" id="templateContent"><?php echo $open; ?></textarea>
	</td></tr>
	</table>

	<script type="text/javascript">
	function submitbutton() {
		$('templateContent').value = editor.getCode();
		$each( $ES( 'input[type=hidden]', 'adminForm' ), function( el ) { if( el.name == 'task' ) el.value = 'saveConfig' } );
		$('adminForm').submit();
	}
	var editor = CodeMirror.fromTextArea('templateContent', {
		...
		stylesheet: ["<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>css/xmlcolors.css", 
			    "<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>css/jscolors.css", 
			    "<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>css/csscolors.css",
			    "<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>contrib/php/css/phpcolors.css"],
		path: "<?php echo "{$config->liveSite}".SOBI_CODEEDITORPATH;?>js/",	
	   	continuousScanning: 500		  	
	});
	</script>
...
?>
		<table class="SobiAdminForm" width="100%">
		<tr>
			<th><?php echo _SOBI2_MENU_EDIT_VC_TEMPLATE ?></th>
		</tr>
		<tr class="row0">
			<td><span class="editlinktip"><?php echo _SOBI2_CHOOSE_TPL_TO_EDIT ?></span>&nbsp;
			<?php echo sobi2AdmHelper::templatesChooser( $tpl, 'stpl', 'vc', array( 'onchange' => 'window.location=window.location+\'&stpl=\'+this.options[this.selectedIndex].value '), false ); ?></td>
		</tr>
		<tr class="row1">
			<td><span class="editlinktip"><?php echo sobiHTML::toolTip(addslashes(_SOBI2_VC_TEMPLATE_ENABLE_EXPL),addslashes(_SOBI2_VC_TEMPLATE_ENABLE),'','',_SOBI2_VC_TEMPLATE_ENABLE, '#',0 );?></span>&nbsp;
			<?php echo sobiHTML::yesnoRadioList( 'useDetailsView', 'class="inputbox"', $config->useDetailsView ); ?></td>
		</tr>
		<tr class="row0">
...
<?php
	...
	function editFormTemplate( &$config )
	{
		...
		<table class="SobiAdminForm" width="100%">
			<tr>
				<th><?php echo _SOBI2_MENU_EDIT_FORM_TEMPLATE ?></th>
			</tr>
			<tr class="row0">
				<td><span class="editlinktip"><?php echo sobiHTML::toolTip(addslashes(_SOBI2_VC_TEMPLATE_ENABLE_EXPL),addslashes(_SOBI2_VC_TEMPLATE_ENABLE),'','',_SOBI2_VC_TEMPLATE_ENABLE, '#',0 );?></span>&nbsp;
				<?php echo sobiHTML::yesnoRadioList( 'useFormTpl', 'class="inputbox"', $config->useFormTpl ); ?></td>
			</tr>
			<tr class="row1">
				<td>... <?php echo str_replace( _SOBI_CMSROOT, null, $templatefile ); echo _SOBI2_GENERAL_FILE_IS; ?><?php echo is_writable($templatefile) ? _SOBI2_GENERAL_FILE_WRITABLE : _SOBI2_GENERAL_FILE_UNWRITABLE; ?></td>
			</tr>
	...
	<table>
		<tr>
			...
			<td style="text-align:left; width: 55%;">
				<select name="mailTpl" class="text_area" onchange="loadMailFram( this.options[this.options.selectedIndex].id, this.options[this.options.selectedIndex].text );" >
					<option><?php echo _SOBI2_SELECT; ?></option>
					<option id="emailsFooter"><?php echo _SOBI2_CONFIG_FOOTER; ?></option>
					<optgroup label="<?php echo _SOBI2_EMAIL_ON_SUBMIT_OPTGR; ?>">
						<option id="emailOnSubmit"><?php echo _SOBI2_EMAIL_ON_SUBMIT; ?></option>
						<option id="emailOnSubmitAdmin"><?php echo _SOBI2_EMAIL_ON_SUBMIT_ADMIN; ?></option>
					</optgroup>
					<optgroup label="<?php echo _SOBI2_EMAIL_ON_UPDATE_OPTGR; ?>">
						<option id="emailOnUpdate"><?php echo _SOBI2_EMAIL_ON_UPDATE; ?></option>
						<option id="emailOnUpdateAdmin"><?php echo _SOBI2_EMAIL_ON_UPDATE_ADMIN; ?></option>
					</optgroup>
					<optgroup label="<?php echo _SOBI2_EMAIL_ON_RENEW_OPTGR; ?>">
						<option id="emailOnRenew"><?php echo _SOBI2_EMAIL_ON_RENEW; ?></option>
						<option id="emailOnRenewAdmin"><?php echo _SOBI2_EMAIL_ON_RENEW_ADMIN; ?></option>
					</optgroup>
					<optgroup label="<?php echo _SOBI2_EMAIL_ON_PAYMENT_OPTGR; ?>">
						<option id="emailOnPayment"><?php echo _SOBI2_EMAIL_ON_PAYMENT; ?></option>
						<option id="emailOnPaymentAdmin"><?php echo _SOBI2_EMAIL_ON_PAYMENT_ADMIN; ?></option>
					</optgroup>
					<optgroup label="<?php echo _SOBI2_EMAIL_ON_APPROVE_OPTGR; ?>">
						<option id="emailOnApprove"><?php echo _SOBI2_EMAIL_ON_APPROVE; ?></option>
					</optgroup>
				</select>
...
<?php
	...
	function generalConfig($task)
	{
		...
		switch($task) {
			...
			break;
			case 'vercheck':
				$title = _SOBI2_SYSTEM.": <small> ["._SOBI2_MENU_VER_CHECKER." ] </small>";
				$style = "cpanel";
			break;
			case 'errlog':
				$title = _SOBI2_SYSTEM.": <small> ["._SOBI2_MENU_ERRLOG." ] </small>";
				$style = "cpanel";
			break;
			case 'registry':
			...
			break;
			case 'syscheck':
				$title = _SOBI2_SYSTEM.": <small> ["._SOBI2_TOOLBAR_GEN_DEB_REP." ] </small>";
				$style = "syscheck";
			break;
			case 'templates':
								...
								while (!feof($fcontent)) {
									...
									elseif ($count == 6) {
										...
										if(stristr($buffer,"(Error number:")) {
											...
											$error->level = sobi2_ereg_replace("[^0-9]","",$error->level);
										}
										else
											$error->level  = 0;
										switch ($error->level) {
    ...
    function pluginConfig( $pluginName )
    {
	    ...
	    ?>
			<table class="SobiAdminHeading" style="margin-top:10px; margin-left:5px; margin-bottom: 10px;">
				<tr>
					<th class="modules" > &nbsp; <small><?php echo 'Please select an entry from the menu to continue ...' ?></small></th>
				</tr>
			</table>
	    <?php
		}
