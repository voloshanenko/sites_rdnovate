...
if ($menu_dtree) {
	if (false) {
	  ...
	  build_tree_menu(1, $database, $config);
	  echo "<link rel=\"stylesheet\" href=\"/modules/mod_sobi2menu/menu.css\" />
	  <script type=\"text/javascript\" src=\"/modules/mod_sobi2menu/jquery.js\"></script>
	  <script type=\"text/javascript\" src=\"/modules/mod_sobi2menu/sobi_menu.js\"></script>
	  <script type=\"text/javascript\">$(document).ready(function(){mainmenu();});</script>";
}
...
function build_tree_menu($var_parent_id = 1, $db, $cgf)
{
	$query = "SELECT relations.catid, name, ordering " .
          ...
          "WHERE published = 1 AND relations.parentid = '{$var_parent_id}' ".
          "ORDER BY {$cgf->catsOrdering}";
	$db->setQuery( $query );
	$categoryList = $db->loadObjectList();
	if(sizeof($categoryList) > 0)
	{
		echo "<ul class='sobi-menu'>\n";
		foreach($categoryList as $category)
		{
			$href = sobi2Config::sef("index.php?option=com_sobi2&amp;catid={$category->catid}&amp;Itemid={$cgf->sobi2Itemid}");
			$name = $cgf->getSobiStr($category->name);
			echo "<li><a href='{$href}'>{$name}</a>\n";
			build_tree_menu($category->catid, $db, $cgf);
			echo "</li>\n";
		}
		echo "</ul>\n";
	}
