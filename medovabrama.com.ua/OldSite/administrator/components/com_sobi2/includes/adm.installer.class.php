<?php
/**
* @version $Id: adm.installer.class.php 4994 2009-04-03 14:35:27Z Radek Suski $
...
class sobi2Installer {
    		...
    		if($fileExt == 'zip') {
    			...
    			if(@move_uploaded_file($file['tmp_name'], $path.strtolower($packageName.'.zip'))) {
    				...
    				if(!$zipfile->extract(PCLZIP_OPT_PATH, $path)) {
						...
						$joomlas = $root->getElementsByPath('joomla_files',1);
						if ($joomlas) {
							$all_joomlas = array();
							if($joomlas->hasChildNodes()) {
	 							$all_joomlas = $joomlas->childNodes;
								if(count($all_joomlas) != 0) {
									foreach($all_joomlas as $joomla_file) {
										$jcFile = $joomla_file->getText();
										$jcLang = $joomla_file->getAttribute("lang");
										if( !file_exists( _SOBI_CMSROOT.DS.'administrator'.DS.'language'.DS.$jcLang ) ) {
											mkdir( _SOBI_CMSROOT.DS.'administrator'.DS.'language'.DS.$jcLang );
										}
										if ( !$jcFile || ( !copy($path.$jcFile, _SOBI_CMSROOT.DS.'administrator'.DS.'language'.DS.$jcLang.DS.$jcFile ) ) ) {
											;
										}
									}
								}
    ...
    function removePlugin( $pid = 0, $pTables = null )
    {
    	...
    	if( $pid ) {
			...
			/** removing files */
			if( file_exists( $config->S2_pluginsAdmPath.DS.$plugin.DS.'uninstall.php' ) ) {
				require( $config->S2_pluginsAdmPath.DS.$plugin.DS.'uninstall.php' );
			}
	...
	function removeSobi()
	{
		...
		if ( class_exists( 'JURI' ) && defined( '_JEXEC' ) )
			$componentpath = "index.php?option=com_installer&type=components&task=manage";
		else
			$componentpath = "index2.php?option=com_installer&element=component";

		sobi2Config::debOut( $t );
		if ( $ok ) {
			sobi2Config::redirect( $componentpath, _SOBI2_DB_REMOVED_MSG );
		}
		else {
			sobi2Config::redirect( $componentpath, _SOBI2_DB_REMOVE_ERROR_MSG );
		}
	}
}
?>