<?php
/**
* @version $Id: admin.field.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class field extends sobiField {
    ...
    function getDataFromRequest()
    {
			...
			$this->fieldname = sobi2_ereg_replace( '[^a-zA-Z0-9_]', null, $this->fieldname );
    ...
    function saveListValues()
    {
		...
		if( empty( $this->definedValues ) ) {
			trigger_error("field::saveListValues(): values array is still empty", E_USER_NOTICE);
			return false;
		}
    ...
    function updateField( $lang = null )
    {
		...
		if( $database->query() ) {
			$config->getEditForm();
			$query = "SELECT COUNT(*) FROM `#__sobi2_language` WHERE `langKey` = '{$this->fieldname}' AND `fieldid` = '{$this->fieldid}' AND `sobi2Lang` = '{$lang}' LIMIT 1";
			$database->setQuery( $query );
			if($database->loadResult() != 0) {
				$statement = "UPDATE `#__sobi2_language` SET " .
						...
						"WHERE `langKey` = '{$this->fieldname}' AND `fieldid` = '{$this->fieldid}' AND `sobi2Lang` = '{$lang}' LIMIT 1";
			}
			else {
